#guys this is a code to includes wtsp it feature too, that there is if we put a msg blank to anyone, so it is not work's.
#and its too that if we put msg just like many more space and then our msg,so its managing this msg
...
# this is #theCODE, yehhh because only it's provides a the best concept.....
...
#this is for check, in our msg space is available in a last or not!
def toCheckSpace(have1):
        k=len(have1)
        if(have1[k-1]!='\n'):
                have1=have1+'\n';
        return have1
...
#this is a to check, legality wtsp feature, like in a msg not only space, not blank, and not msg first many spaces then msg's.....
def weCheckSpace(have1):
        if(have1=='\n'):
                return 0;
        k=0;
        for char in have1:
...
def find(your,another):
        ...
        specialCount=yourValue=anotherValue=0; k=1
        while(1):
                s=findHowsLines(k,file)
                if(s[0]!=k):
                        have1=s[1]
                        have1=toCheckSpace(have1);
                        l=-1;
                        for have2 in have1:
                                if(have2==':'):
                                        break;
                                l+=1;
                        if(have1[:l]==your):
                                have1=weCheckSpace(have1[l+3:])
                                if(have1==0):
                                        continue;
                                sender(have1,l,0 if(specialCount==yourValue) else 1,1 if(specialCount==0) else 0);
                                yourValue=specialCount=str(int(specialCount)+1);
                        elif(have1[:l]==another):
                                have1=weCheckSpace(have1[l+3:])
                                if(have1==0):
                                        continue;
                                reciever(have1,another,0 if(specialCount==anotherValue) else 1,1 if(specialCount==0) else 0);
